...
interface WallFinishingsModalProps {
  isDarkMode: boolean;
...
const WallFinishingsModal = ({
  ...
  return (
    <Draggable handle="strong">
      ...
      >
        <div className="wall-color-header">
          <span className="wall-color-title">{title || "Wall color"}</span>
          <img
            ...
            >
              {finishingCategories.map((category: any) => {
                return (
                  <option
                    key={category._id}
                    className="wall-option-container"
                    value={category._id}
                  >
...
export default WallFinishingsModal;
