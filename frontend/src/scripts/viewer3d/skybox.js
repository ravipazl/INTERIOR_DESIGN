...
export class Skybox extends EventDispatcher {
  constructor(scene, renderer) {
    ...
    this.bottomColor = 0xd8ecf9; //0xD8ECF9
    ...
    // loader.load('./assets/icons/bg.jpg' , function(texture)
    //     {
  ...
  __createGridFloors() {
    ...
    this.__fineGridFloor = new GridHelper(
      this.__gridSize,
      Math.round(this.__gridSize / Configuration.getNumericValue(gridSpacing)),
      0x0f0f0f,
      0x808080
    );
    this.__coarseGridFloor = new GridHelper(
      this.__gridSize,
      Math.round(
        this.__gridSize / (Configuration.getNumericValue(gridSpacing) * 5)
      ),
      0x0f0f0f,
      0x303030
    );
  ...
  setEnvironmentMap(url) {
    ...
    scope.texture.load(
      ...
      undefined,
      function () {
        console.debug("ERROR LOADEING FILE");
      }
    );
  }
