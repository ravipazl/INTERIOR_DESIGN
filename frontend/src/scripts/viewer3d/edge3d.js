import { EventDispatcher, AxesHelper, Vector2, Vector3, MeshBasicMaterial, FrontSide, DoubleSide, BackSide, Shape, Path, ShapeGeometry, Mesh, Geometry, Face3, Box3, MeshPhongMaterial } from 'three';
...
export class Edge3D extends EventDispatcher {
    constructor(scene, edge, controls, opts) {
        ...
        this.fillerColor = 0xdddddd; //0xdddddd;
        this.sideColor = 0xcccccc; //0xcccccc;
        this.baseColor = 0xdddddd; //0xdddddd;
        this.visible = true;
    ...
    __updateTexturePack(evt) {
        if (evt.type === EVENT_UPDATE_TEXTURES) {
            ...
            // COOHOM-STYLE GREY WALLS — for the DEFAULT wall texture ONLY. The
            // default is the cream "solid" image (21051 / 21321) that tints
            // walls tan/green; render just those as flat grey (work on a COPY so
            // saved data is untouched). IMPORTANT: match ONLY those two default
            // files — NOT every "/solids/" texture — otherwise any wall colour
            // the user picks from Wall Properties (21003, 21004, …) would also
            // be forced grey and the colour picker would appear broken.
            if (
                texturePack &&
                typeof texturePack.colormap === 'string' &&
                (texturePack.colormap.indexOf('21051') !== -1 ||
                    texturePack.colormap.indexOf('21321') !== -1)
            ) {
                texturePack = { ...texturePack, color: '#E0E0E0' };
                delete texturePack.colormap;
            }
            ...
            if (!this.__wallMaterial3D) {
                ...
                this.__wallMaterial3D = new WallMaterial3D({ color: texturePack.color, side: side, transparent: false, wireframe: false }, texturePack, this.scene);
            }
    ...
    addToScene() {
        ...
        this.updateVisibility();
        this.applyWallOpacity();
    }

    // Re-apply the viewer's wall opacity on every (re)draw, so adding furniture /
    // editing never resets the walls back to solid. viewer.__wallOpacity < 1 →
    // semi-transparent walls (Coohom's "Wall %" slider).
    applyWallOpacity() {
        let scope = this;
        try {
            let op = scope.scene && scope.scene.__wallOpacity;
    ...
    updateVisibility() {
        ...
        // DOLLHOUSE CUTAWAY: hide the walls that face the camera (the near walls
        // between you and the room) and keep the far walls solid — so you look
        // straight INTO the rooms, like Coohom. Walls stay opaque; the *hiding*
