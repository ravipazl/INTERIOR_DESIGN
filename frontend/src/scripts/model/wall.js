...
export class Wall extends EventDispatcher {
   ...
   */
  constructor(start, end, aa, bb, texture) {
    super();
    this.start = start;
    this.end = end;
    this.name = "wall";
    ...
    /** The front-side texture. Clone so this wall side owns its own texture
     * object — otherwise every wall shares the single `defaultWallTexture`
     * instance (or a passed-in one) and changing one wall's colour mutates
     * them all. */
    this.frontTexture = { ...(texture ? texture : defaultWallTexture) };

    /** The back-side texture. Separate clone so front and back are independent. */
    this.backTexture = { ...(texture ? texture : defaultWallTexture) };
