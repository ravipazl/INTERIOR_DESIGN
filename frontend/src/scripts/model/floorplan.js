...
import BlueprintInterface from "../../blueprint-interface.js";
...
export class Floorplan extends EventDispatcher {
  ...
  constructor() {
    ...
    this.floorTextures = {};
    // Per-room ceiling finish (uuid → { color, textureUrl }); serialized so it
    // survives reload, like floorTextures.
    ...
    this.__updateRoomFloorTextureEvent =
      this.__updateRoomFloorTexture.bind(this);
  ...
  newWallsForIntersections(start, end) {
    ...
    for (var i = 0; i < this.walls.length; i++) {
      ...
      if (iPoint) {
        ...
        console.debug(newCorners, "newCorners");
   ...
   */
  newWall(start, end, a, b, texture) {
    var scope = this;
    var wall = new Wall(start, end, a, b, texture);
  ...
  newCorner(x, y, id) {
    ...
    // There should be an update whenever a new corner is inserted
    this.update();
  ...
  saveFloorplan() {
    ...
    floorplans.ceilingTextures = this.ceilingTextures;
    return floorplans;
  }
  ...
  loadFloorplan(floorplan) {
    ...
    floorplan.walls.forEach((wall, index) => {
      let newWall = scope.newWall(
        ...
        null,
        null,
        index % 2 === 0 ? defaultWallTexture : defaultWallTexture2
      );

      if (wall.frontTexture) {
        // Clone so each wall owns its texture object (no shared singleton/ref
        // that a per-wall colour change would mutate across all walls).
    ...
    if ("ceilingTextures" in floorplan) {
      this.ceilingTextures = floorplan.ceilingTextures || {};
    }
  ...
  __updateRoomFloorTexture(evt) {
    console.debug("__updateRoomFloorTexture ~ evt", evt);
    let textures = {};
    this.rooms.map((room) => {
      const floorTexture =
      ...
      if (floorTexture) {
        textures[room.uuid] =
          room.uuid === evt.selectedRoom.uuid
            ? { ...floorTexture, colormap: evt.selectedTexture }
            : floorTexture;
      }
    });
    this.floorTextures = textures;
  }
  ...
  // Per-room ceiling finish (color / texture url). Stored on the floorplan so it
  // serializes with the design — like floor textures — and survives reload.
  getCeilingTexture(uuid) {
    if (this.ceilingTextures && uuid in this.ceilingTextures) {
      return this.ceilingTextures[uuid];
    }
    return null;
  }

  setCeilingTexture(uuid, data) {
    if (!this.ceilingTextures) this.ceilingTextures = {};
    this.ceilingTextures[uuid] = data;
  }
  ...
  removeLastCorner() {
    var tmpCorners = this.corners;
    var tmpWalls = this.walls.slice(0);
    tmpCorners.forEach((corner, index) => {
      if (index == tmpCorners.length) {
        corner.remove();
      }
    });
    tmpWalls.forEach((wall) => {
