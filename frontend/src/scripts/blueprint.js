...
class BlueprintJS {
  ...
  setViewer2DModeToDraw(mode) {
    console.debug(this.options.widget, "mode");
    if (this.options.widget) {
      return;
    }
  ...
  AddNewItems(item) {
    this.roomplanner.__addNewItem(item);
  }
  ...
  toggleThemeMode() {
    ...
    // Warm soft backdrop (light) / deep neutral (dark) — not stark white/grey.
    const colorCode = isDarkMode ? 0x191a1e : 0xf2efea;
    this.floorplanner.setBackgroundColor(colorCode);
    if (this.roomplanner && this.roomplanner.setBackgroundColor) {
      this.roomplanner.setBackgroundColor(colorCode);
    }
    this.floorplanningHelper.__floorplanner.__options.dimtextcolor = isDarkMode
      ? "#FFFFFF"
      : "#333333";
    this.floorplanningHelper.__floorplan.update();
  }
}
