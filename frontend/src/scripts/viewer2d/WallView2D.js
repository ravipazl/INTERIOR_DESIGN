...
export class WallDimensions2D extends Graphics {
    ...
    __updateDimensionText() {
        ...
        //util
        // if (Configuration.getData().dimUnit === "mm") {
        //     //showing the labels in terms of meters even though the unit is mm
        //     this.__textfield.text = Dimensioning.cmToMeasureUnit(this.__wall.wallSize,1, 'm');
        // }
        // if (Configuration.getData().dimUnit === "feetAndInch") {
        //     this.__textfield.text = Dimensioning.cmToMeasureUnit(this.__wall.wallSize,1, 'feetAndInch');
        // }
        this.__textfield.text = dimUnitWallLabel(this.__wall.wallSize);
        // console.log("textfield wallview 2d ", this.__textfield.text);
...
export class WallView2D extends BaseFloorplanViewElement2D {
    constructor(floorplan, options, wall) {
        ...
        // Show-on-select: the per-wall dimension label is HIDDEN by default so
        // the plan isn't a wall of overlapping numbers. It's revealed only for
        // the wall the user selects (see `set selected`).
    ...
    set viewDimensions(value) {
        ...
        this.__info.alpha = 0.5;
    }

    // Show/hide the wall-mounted dimension label entirely. This is separate from
    // viewDimensions (which only controls dull/bright alpha). Used by the 2D
    // viewer's "Inner" measurement toggle so the per-wall labels appear ONLY
    // when the user asks for them, instead of always being on screen.
    set dimensionsVisible(flag) {
        if (this.__info) this.__info.visible = !!flag;
    }

    get dimensionsVisible() {
        return !!(this.__info && this.__info.visible);
    }
    ...
    __drawPolygon(color = 0xDDDDDD, alpha = 1.0) {
        this.clear();
        // Draw the wall as ONE solid bar spanning its full thickness — a single
        // combined wall for the two adjacent rooms — instead of two separate
        // half-edge face polygons, which render as two face lines and read as a
        // "double wall". Wall thickness/geometry, the 3D view and the BOQ are all
        // unchanged; only the 2D fill is unified. The per-edge graphics are cleared
        // so only the single solid bar shows.
        if (this.__frontEdge) {
            this.__frontEdge.clear();
        }
        if (this.__backEdge) {
            this.__backEdge.clear();
        }
        let start = this.__wall.start.location.clone();
        let end = this.__wall.end.location.clone();
        let dir = end.clone().sub(start);
        if (dir.length() === 0) {
            return;
        }
        dir.normalize();
        let half = (this.__wall.thickness || 0) * 0.5;
        let offset = new Vector2(-dir.y, dir.x).multiplyScalar(half);
        let p1 = Dimensioning.cmToPixelVector2D(start.clone().add(offset));
        let p2 = Dimensioning.cmToPixelVector2D(end.clone().add(offset));
        let p3 = Dimensioning.cmToPixelVector2D(end.clone().sub(offset));
        let p4 = Dimensioning.cmToPixelVector2D(start.clone().sub(offset));
        this.lineStyle(0);
        this.beginFill(color, alpha);
    ...
    __drawHoveredOffState() {
        // Visual difference by WALL TYPE (thickness preset — see the Wall type
        // dropdown in PropertiesPanel; thresholds match wallTypeOf()):
        //   partition (thin, <=12cm)  -> grey solid bar
        //   structural (thick, >=16cm) / custom -> black solid bar
        // Combined with the thickness itself, structural reads as a heavy black
        // wall and partition as a lighter, thinner grey wall. Hover/selected
        // states keep their own highlight colours.
        let thickness = this.__wall ? this.__wall.thickness : 0;
        let isPartition = thickness <= 12;
        let color = isPartition ? 0x8a8f98 : 0x000000;
        // Softer solid black for structural/custom walls (reduced opacity);
        // partition grey stays full strength.
        let alpha = isPartition ? 1.0 : 0.75;
        this.__drawPolygon(color, alpha);
    }
    ...
    set selected(flag) {
        ...
        // NOTE: label VISIBILITY (show-on-select vs. show-all) is reconciled
        // centrally by the 2D viewer (Viewer2D.__applyWallDimVisibility), which
        // knows the Inner/Outer button state. Here we only set the alpha.
    }
}